...
- `chrome-troubleshooting`: if Dia extension setup, installation, or communication fails, you MUST immediately emit and read this in full before retrying, inspecting scripts, trying alternate browser selectors, or taking any other recovery action. Use its extension and native-host guidance only where it also applies to Dia; do not use Google Chrome-only launch, running, profile, or recovery checks for Dia.
...
- At the start of every Dia browser task, call `await browser.nameSession("...")` immediately after setup and before opening or claiming tabs. Use a short task name that starts with a neutral, friendly, task-relevant emoji; if unsure, use 🔎.
...
* You are provided with various options for interacting with Dia through the browser-client runtime, including Playwright and vision. Use the most appropriate tool for the job.
